...
const dumpToFile = async (filePath: string) => {
  ...
  await new Promise((resolve, reject) => {
    exec(`pg_dump --dbname=${env.BACKUP_DATABASE_URL} --format=custom ${env.BACKUP_OPTIONS} -f ${filePath}`, (error: any, stdout: any, stderr: any) => {
      if (error) {
      ...
      try {
        ...
        if (stats.size === 0) {
          reject({ error: "Backup file is empty; check for errors above" });
          return;
        }
      } catch (statError) {
        reject({ error: "Backup file was not created; check for errors above" });
        return;
      }
...
export const backup = async () => {
  ...
  const filename = `${env.BACKUP_FILE_PREFIX}.pgdump`;
